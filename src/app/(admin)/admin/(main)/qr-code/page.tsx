...
const selections = [{ label: "Open", key: 1 }];
...
export default function ManageQrCodes() {
  ...
  const huntsQuery = useQuery(
    ["qr-codes", page],
    ({ queryKey }) => {
      return axiosInstance.get(`/qr-code/all?page=${queryKey[1]}&limit=10`);
    },
    {
      onSuccess(data) {
        console.log(data);
  ...
  const deleteHunt = useMutation(
    (data: string): any => axiosInstance.delete(`/qr-code?id=${data}`),
    {
      onSuccess(data: any) {
    ...
    <>
      <>
        <div className="flex justify-between items-center">
          <p className="text-xl font-semibold">Manage Qr Codes</p>
          <Link
            href={"/admin/qr-code/create"}
          ...
          {!huntsQuery.isFetching && (
            <>
              <table className="p-4 w-full block sm:table overflow-auto mt-4">
                ...
                <tbody>
                  {huntsQuery.data?.data.data.map((e: any, index: number) => (
                    <tr
                    ...
                    >
                      ...
                      <td className="p-2 text-sm">
                        <div className="flex gap-2">
                          ...
                          <Link href={`/admin/qr-code/view?id=${e._id}`}>
                            <GrView
                              onClick={() => {
                                // downloadPdf.mutate(e._id);
                                // setRiddleToView(e)
                          ...
                          <Link href={`/admin/qr-code/edit?id=${e._id}`}>
                            <CiEdit
                              // onClick={() => {
              ...
              <div className="flex flex-wrap gap-4">
                {!!huntsQuery.data?.data.lastPage &&
                  huntsQuery.data?.data.lastPage != page && (
                    <button
      ...
      >
        <ModalContent>
          {(onClose) => (
            <>
              <ModalHeader className="flex flex-col text-xl gap-1">
                Riddles Created Successfully
              </ModalHeader>
              <ModalBody className="flex flex-col gap-4 pb-8">
                <p className="text-sm text-gray-400">
                  New Riddle has been created successfully & QR Code is
                  generated
                </p>
