    ...
    return (
        <>
                ...
                <div className="flex justify-between items-center">
                    ...
                    <Link href={'/admin/manage-breweries/create'} className="sm:px-16 px-4 py-2 bg-[#A92223] flex justify-center rounded text-white w-max ">Add Brewery</Link>
                </div>
                        ...
                        <>
                            ...
                            <ModalBody className="flex flex-col gap-4 pb-8">
                                <div className="flex w-full flex-wrap gap-4 h-[15rem] overflow-auto">
                                    ...
                                    {breweryName?.schedule?.map((f: any,index:number) => <div className="p-4 border-2 rounded-lg flex flex-col gap-4">
                                        <Input
                                            label="Day"
                                            disabled
                                            defaultValue={`${f.day}`}
                                        ...
                                        <TimeInput defaultValue={new Time(parseInt(f.time.start.split(':')[0]), parseInt(f.time.start.split(':')[1]))} label="Starting Time" labelPlacement="outside" classNames={{ label: "!font-semibold" }} isRequired onChange={(e) => {
                                        ...
                                        <TimeInput defaultValue={new Time(parseInt(f.time.end.split(':')[0]), parseInt(f.time.end.split(':')[1]))} label="Ending Time" labelPlacement="outside" classNames={{ label: "!font-semibold" }} onChange={(e) => {
                                            const findDay = breweryName.schedule.find((e: any, index1: number) => index1 == index)
                                            const startHour = `${e.hour < 10 ? `0${e.hour}` : `${e.hour}`}`
                                            const startMinute = `${e.minute < 10 ? `0${e.minute}` : `${e.minute}`}`
