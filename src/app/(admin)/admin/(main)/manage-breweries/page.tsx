...
export default function ManageBreweries() {
  ...
  const deleteHunt = useMutation(
    (data: string): any => axiosInstance.delete(`/brewery?breweryId=${data}`),
    {
      onSuccess(data: any) {
  ...
  const googleMapsQuery = useQuery(
    ["googlemapsGeocode", location],
    ({ queryKey }) =>
      axios.get(
        `https://maps.googleapis.com/maps/api/geocode/json?address=${queryKey[1]}&key=${process.env.NEXT_PUBLIC_GOOGLEAPI}`
      ),
    {
      onSuccess(data) {
  ...
  const breweryEditMutation = useMutation(
    (data: BreweryEditData) =>
      axiosInstance.put(`/brewery?breweryId=${breweryName._id}`, data),
    {
      onSuccess(data) {
  ...
  const getLocationPdfMutation = useMutation(
    (data: any) =>
      axiosInstance({
        method: "GET",
        url: `/admin/stats-user?breweryId=${breweryForPDf._id}${
    ...
    <>
      <>
        <div className="flex justify-between items-center">
          ...
          <Link
            href={"/admin/manage-breweries/create"}
          ...
          <>
              ...
              <tbody>
                {breweryQuery.data?.data.data.map((e: any, index: number) => (
                  ...
                  >
                    ...
                    <td className="p-2 text-sm">
                      <Image
                        className="w-[3rem] h-[3rem] object-contain bg-[#160704] rounded-full p-2"
                        src={
                          e.media
                            ? e.media.includes("placeholder")
                              ? "/images/user/profile/profile.png"
                              : `${process.env.NEXT_PUBLIC_MEDIA_URL}/${e.media}`
                            : "/images/user/profile/profile.png"
                        }
                        alt="brewery Logo"
                        width={100}
                    ...
                    <td className="p-2 text-sm">
                      <div className="flex gap-2">
                        <Link href={`/admin/manage-breweries/edit?id=${e._id}`}>
                          <CiEdit className=" cursor-pointer border-[0.15rem] text-4xl text-red-600 rounded-lg p-2 border-red-600" />
                        </Link>
...

            {
              <div className="flex flex-wrap gap-4">
                {!!breweryQuery.data?.data.lastPage &&
                  breweryQuery.data?.data.lastPage != page && (
                    <button
              ...
              <ModalBody className="flex flex-col gap-4 pb-8">
                <div className="flex w-full flex-wrap gap-4 h-[15rem] overflow-auto">
                  ...
                  <Autocomplete
                    ...
                    items={
                      googleData && !!location ? googleData : [{ label: "" }]
                    }
                    defaultItems={[{ label: "" }]}
                    className="w-full !font-semibold"
                    // allowsCustomValue={true}
                    onSelectionChange={(key) => {
                      const finding = googleData.find(
                        (e: any) => e.label == key
                      );
                      console.log(finding);
                  ...
                  {breweryName?.schedule?.map((f: any, index: number) => (
                    <div className="p-4 border-2 rounded-lg flex flex-col gap-4">
                      ...
                      <TimeInput
                        defaultValue={
                          new Time(
                            parseInt(f.time.start.split(":")[0]),
                            parseInt(f.time.start.split(":")[1])
                          )
                        }
                        label="Starting Time"
                        labelPlacement="outside"
                        ...
                        isRequired
                        onChange={(e) => {
                          const findDay = breweryName.schedule.find(
                            (e: any, index1: number) => index1 == index
                          );
                          const startHour = `${
                            e!.hour < 10 ? `0${e!.hour}` : `${e!.hour}`
                          }`;
                          const startMinute = `${
                            e!.minute < 10 ? `0${e!.minute}` : `${e!.minute}`
                          }`;
                          findDay!.time.start = `${startHour}:${startMinute}`;
                          const newSchedule = breweryName.riddles.map(
                            (p: any, index1: number) => {
                              if (index1 == index) {
                                return findDay;
                              }
                      ...
                      <TimeInput
                        defaultValue={
                          new Time(
                            parseInt(f.time.end.split(":")[0]),
                            parseInt(f.time.end.split(":")[1])
                          )
                        }
                        label="Ending Time"
                        labelPlacement="outside"
                ...
                <button
                  ...
                  className="px-16 py-2 bg-[#A92223] w-max rounded text-white"
                >
                  {breweryEditMutation.isLoading ? (
        ...
        <ModalContent>
          {(onClose) => (
            <>
              ...
              <ModalBody className="flex flex-col gap-4 pb-8">
                <form
                  onSubmit={handleSubmit(SubmitForPDf)}
                ...
                >
                  <Select
                    ...
                    label="Date Range"
                    classNames={{
                      label: "font-bold !text-[#A92223] ",
                      trigger: "!border-[#A92223] ",
                      selectorIcon: "text-[#A92223]",
                      value: "!text-[#A92223]",
                    }}
                  ...
                  <Button
                    type="submit"
                    isLoading={getLocationPdfMutation.isLoading}
                    isDisabled={getLocationPdfMutation.isLoading}
                    className="sm:px-16 px-4 py-2 bg-[#A92223] flex justify-center rounded text-white w-max "
                  >
                    Get Location Stats
                  </Button>
