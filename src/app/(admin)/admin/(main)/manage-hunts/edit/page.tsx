...
export default function EditHunts(data: any) {
    ...
    const deleteRiddle=useMutation((riddleId:string)=>axiosInstance.delete(`/hunt/riddle?huntId=${data.searchParams.id}&riddleId=${riddleId}`),{
        onSuccess(data, variables, context) {
            console.log('deleted riddle',data)
        },
    })
    const updateRiddle=useMutation((datas:any)=>{
        return axiosInstance.put(`/hunt/riddle?huntId=${data.searchParams.id}`,datas)
    },{
        onSuccess(data) {
            ...
            {message && <p className="text-red-600 text-center">{message}</p>}
                    ...
                    <div className="flex flex-col mt-3 gap-4">
                        ...
                        <div className="sm:w-[70%] w-full flex flex-col gap-4 border-[0.1rem] p-4 rounded-lg">
                            ...
                            <Switch isSelected={e.hasReward} onValueChange={(j) => {
                                ...
                                <button onClick={() => {
                                    ...
                                    deleteRiddle.mutate(e.riddleId)
                        ...
                        {newRiddless?.map((e: any, index: number) =>
                            ...
                            <p className="font-semibold text-sm">Title</p>
                            <ReactQuill 
                            placeholder="Enter Riddle Title" value={e.title}
