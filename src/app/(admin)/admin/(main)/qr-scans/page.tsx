  ...
  const bookingsQuery = useQuery(
    ...
    ({ queryKey }) => {
      return axiosInstance.get(
        `/hunt/riddle-scans?page=${queryKey[1]}&limit=10&date=${queryKey[2]}`
      );
    },
    {
      onSuccess(data) {
  ...
  return (
    <>
      <div className="flex justify-between">
        <p className="text-xl font-semibold">QR Riddle Scans</p>
      </div>
          ...
          <table className="p-4 mt-4 w-full block sm:table overflow-auto">
            <thead>
              <tr className="bg-gray-200">
                ...
                <th className="p-2 text-sm text-left">Riddle Title</th>
                <th className="p-2 text-sm text-left">Reward</th>
              ...
              {bookingsQuery.data?.data.data.map((e: any, index: number) => (
                ...
                >
                  ...
                  <td
                    ...
                    dangerouslySetInnerHTML={{ __html: e.riddleInfo.title }}
                  />
                  <td
                    className="p-2 text-sm"
                    dangerouslySetInnerHTML={{ __html: e.riddleInfo.reward }}
                  />
                  <td
                    className="p-2 text-sm"
                    dangerouslySetInnerHTML={{ __html: e.riddleInfo.hint }}
                  />

                  <td className="p-2 text-sm">{e.totalScanned}</td>
                  <td className="p-2 text-sm">
                    {new Date(e.riddleInfo.createdAt).toLocaleDateString()}
                  </td>
