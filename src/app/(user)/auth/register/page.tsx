...
export default function Register() {
  ...
  const registerMutation = useMutation(
    (data: LoginData) => axiosInstance.post("/auth/signup", data),
    {
      onSuccess(data) {
        ...
        localStorage.setItem(
          "refreshToken",
          data.data.data.tokens.refresh_token
        );
        localStorage.setItem(
          "userData",
          JSON.stringify({
        ...
        (window as any).fbq("track", "AccountCreation", {
  ...
  const loginMutation = useMutation(
    (data: LoginData): any => axiosInstance.post("/auth/login", data),
    {
      onSuccess(data: any) {
  ...
  const { field: field1, fieldState: fieldState1 } = useController({
    name: "above21",
    control,
    ...
    <>
      ...
      >
        ...
        <div className="flex w-full justify-between">
          <CheckboxGroup
            isInvalid={!!fieldState1.error}
            errorMessage={fieldState1.error?.message}
            {...field1}
          >
            <Checkbox value={"yes"}>
        ...
        <button
          type="submit"
          className="bg-[#A92223] rounded-lg p-4 text-white w-[80%]"
        >
          SignUp
        </button>
