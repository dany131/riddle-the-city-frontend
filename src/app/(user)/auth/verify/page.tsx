    ...
    return (
        <>
            <form onSubmit={handleSubmit}
                  className="h-full overflow-auto w-full items-center  hidden sm:flex flex-col gap-4 p-8 sm:px-24 sm:py-8 ">
                <h1 className="text-2xl font-bold">Verification</h1>
                {invalid && <p className="text-red-600">{message}</p>}
                <p className="text-center">We have sent you an email containing verification code. Please enter the code
                ...
                {code.length == 4 && numbers.includes(code[3]) &&
                    <button className="bg-[#A92223] rounded-lg px-16 py-2 text-white w-full"
                            type="submit">Submit</button>}

                <p className="text-center" hidden={disabled}>
                ...
                {/*    // setIsPlaying(true)*/}
                {/*    resendVerificationQuery.mutate();*/}
                {/*}} disabled={disabled} className="bg-[#A92223] rounded-lg px-16 py-2 text-white w-full"*/}
                {/*        type="button">Resend Code*/}
                {/*</button>*/}
            ...
            <form onSubmit={handleSubmit} className="h-full overflow-auto w-full bg-[#160704] sm:hidden relative">
                ...
                <div className="flex flex-col relative z-[1] h-[100vh] text-white gap-4 p-8 sm:p-24 items-center">
                    <h1 className="text-2xl font-bold">Verification</h1>
                    {invalid && <p className="text-red-600">{message}</p>}
                    <p className="text-center">We have sent you an email containing verification code. Please enter the
                        code to verify your identity</p>
                    <ReactInputVerificationCode onChange={(value) => {
                        setCode(value);
                    }} length={4}/>
                    <CountdownCircleTimer
