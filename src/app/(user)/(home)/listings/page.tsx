...
export default function Listings() {
  const {
    ...
    queryKey: ["allBrwery"],
    queryFn: ({ pageParam = 1 }) =>
      axiosInstance.get(`/brewery/all?page=${pageParam}&limit=5`),
    getNextPageParam: (lastPage: any, pages) =>
      lastPage.data.page != lastPage.data.lastPage
      ...
      <div className="flex flex-col Voigante">
        <div className="min-h-[70vh] relative">
          ...
          <div className="pt-[16rem] sm:pt-72 md:pt-56 z-[999] relative flex flex-col md:flex-nowrap flex-wrap gap-4 px-8 sm:px-28 pb-16 ">
            <p>
              Home <span className="mx-2">/</span> Listings
            </p>
                ...
                <div className="sm:w-[70%] flex flex-wrap border-1 border-[#E2CEAB] sm:flex-nowrap bg-[#2A1B15] rounded-lg">
                  <div className=" w-full max-h-[20rem] p-2 sm:w-[40%]">
                    <Image
                      ...
                      src={`${process.env.NEXT_PUBLIC_MEDIA_URL}/${j.media}`}
                      width={200}
                      height={300}
                      alt="store-1"
                    />
                  ...
                  <div className="w-full px-4 sm:w-[60%] sm:px-16 py-8 gap-4 flex flex-col">
                    ...
                    {/* <p className={`${montesserat.className}`}>They create ales that they are passionate about
                                and crave.</p> */}
                    <div
                      className={`flex items-center gap-8 ${montesserat.className}`}
                    >
                          ...
                          <>
                            <div className="flex gap-2 flex-wrap">
                              ...
                              <p className="font-semibold">
                                {`${
                                  parseInt(p.time.start.split(":")[0]) % 12 ||
                                  12
                                }:${p.time.start.split(":")[1]}${
                                  parseInt(p.time.start.split(":")[0]) < 12
                                    ? "AM"
                                    : "PM"
                                } - 
                           ${parseInt(p.time.end.split(":")[0]) % 12 || 12}:${
                                  p.time.end.split(":")[1]
                                }${
                                  parseInt(p.time.end.split(":")[0]) < 12
                                    ? "AM"
                                    : "PM"
                                }`}
                              </p>
                    ...
                    <div className="w-max h-max relative">
                      <Image
                        src={"/images/layout/Group 10.svg"}
                        className="w-full h-full object-contain absolute z-[4]"
                        alt="link"
                        width={100}
                        height={100}
                      />
                      <button
                        ...
                        onClick={() =>
                          window.open(
                            `${googleMapUrl}/${encodeURIComponent(
                              j.address.text
                            )}`,
                            "_blank"
                          )
