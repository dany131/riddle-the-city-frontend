...
export default function Dashboard() {
  ...
  const breweryQuery = useQuery(
    ["breweries", searchQuery, page],
    ({ queryKey }) => {
      return axiosInstance.get(
        `/brewery/all?page=${queryKey[2]}&limit=10&searchQuery=${queryKey[1]}`
      );
    },
    {
      onSuccess(data) {
  ...
  const userStatsQuery = useQuery(["stats"], () =>
    axiosInstance.get("/user/stats")
  );

  const activeHuntQuery = useQuery(
    ["activeHunt", userStatsQuery.data?.data.data.activeHunt._id],
    ({ queryKey }) => axiosInstance.get(`/hunt?huntId=${queryKey[1]}`),
    {
      enabled: !!userStatsQuery.data?.data.data.activeHunt._id,
    }
  );

  const markRiddleAsCompleted = useMutation(
    () => axiosInstance.put("/hunt/expire"),
    {
            ...
            {[
              {
                label: "Current Pass",
                value:
                  userStatsQuery.data?.data.data.currentPackage.name || "None",
              },
              {
                label: "Riddles Completed",
                value: userStatsQuery.data?.data.data.riddlesCompleted,
              },
              {
                label: "Hunts Completed",
                value: userStatsQuery.data?.data.data.huntsCompleted,
              },
              {
                label: "Active Hunt",
                value: userStatsQuery.data?.data.data.activeHunt.name ? (
                  <p
                    onClick={() => onOpen2()}
                    className="text-[#A92223] cursor-pointer underline"
                  >
              ...
              >
                ...
                <div className="h-[5rem] w-[5rem] bg-gray-200 p-4 rounded-full">
                  <Image
                    ...
                    src={"/images/admin/main/dashboard/drink.svg"}
                    alt="drink"
                    width={100}
                    height={100}
                  />
            ...
            <>
              ...
              <ModalBody className="flex flex-col gap-4 pb-8">
                ...
                <div className="flex w-full gap-4">
                  <Link
                    href={"/startRiddle"}
                    className="px-16 w-max py-2 bg-[#A92223] rounded text-white"
                  >
                    Start Riddle
                  </Link>
                  <Button
                    isLoading={markRiddleAsCompleted.isLoading}
                    isDisabled={markRiddleAsCompleted.isLoading}
                    onClick={() => markRiddleAsCompleted.mutate()}
                    className="px-16 w-max py-2 bg-[#A92223] rounded text-white"
                  >
                    Withdraw Hunt
                  </Button>
        ...
        <ModalContent>
          {(onClose) => (
            <>
              ...
              <ModalBody className="flex flex-col gap-4 pb-8">
                <p className="text-sm">
                  Congratulations! You successfully solved the riddle. You got
                  Reward: 30% discount on a drink.
                </p>
                <div className="flex w-full gap-4">
                  <button className="px-16 w-max py-2 bg-[#A92223] rounded text-white">
                    Claim Reward
                  </button>
                  <button className="px-16 w-max py-2 bg-[#A92223] rounded text-white">
                    Next Riddle
