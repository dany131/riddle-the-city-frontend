        ...
        <>
            {checkoutPage == 0 && <>
                <div className={`flex flex-col h-[100vh] sm:h-full gap-4 sm:px-16 px-4 py-8  ${montesserat.className}`}>
                    ...
                    <div
                        className="flex h-full sm:flex-row flex-col sm:w-1/2 sm:self-center border-1 rounded-lg gap-16 p-4">

                        <div
                            className={`w-full h-full ${!packagesQuery.isFetching ? "overflow-auto" : ""}  flex flex-col gap-4`}>
                            {packagesQuery.isFetching &&
                                <div className="flex justify-center h-full items-center"><ImSpinner2
                            ...
                            {!packagesQuery.isFetching && <>
                                ...
                                <div className="flex justify-between gap-4">
                                    ...
                                    }} className="w-full" label="Coupon" labelPlacement="outside"
                                           placeholder="Enter Code..."/>
                                    <Button onClick={() => {
                                        couponMutation.mutate(code);
                                    }}
                                            className=" w-full sm:w-max m-auto mb-0 px-4 flex justify-center py-2 bg-[#A92223]  rounded text-white">Submit
                                        Code</Button>
                                </div>
                                <div className="flex flex-col gap-2">
                                    <p className="font-semibold">Total Amount</p>
                                    <div className="flex flex-col gap-2">
                                        {couponMutation.data?.data.data && (<div className="flex flex-col gap-2">
                                            <div
                                                className="flex justify-between border-b-[0.1rem] border-dashed border-[#c9c9c9] pb-4">
                                                <p className="text-[#c9c9c9] ">Price</p>
                                                <p className="font-semibold">{`$${couponMutation.data?.data.data.discountedPrice}`}<small
                                                    className="text-red-600">{` ${couponMutation.data?.data.data.coupon.code} applied`}</small>
                                                </p>
                                            </div>
                                        </div>)}
                                        <div className="flex flex-col gap-2">
                                            <div className="flex justify-between">
                                                <p>Tax</p>
                                                <p className="font-semibold">${packagesQuery.data?.data.data.tax}</p>
                                            </div>
                                        ...
                                        <div className="flex flex-col gap-2">
                                            ...
                                            <div className="flex justify-between">
                                                ...
                                                <p className="font-semibold">{couponMutation.data?.data.data ? `$${couponMutation.data?.data.data.finalAmount}` : `$${packagesQuery.data?.data.data.finalAmount}`}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <button type="button" onClick={() => {
                                    setCheckoutPage(1);
                                        ...
                                        className=" w-full sm:w-max m-auto mb-0 px-4 flex justify-center py-2 bg-[#A92223]  rounded text-white">Pay
                                    Now
                                </button>
                            </>}
                        </div>
            ...
            {checkoutPage == 1 && <>
                <div className="flex flex-col gap-4 h-[100vh] sm:h-full sm:px-16 px-4 py-8 ">
                    ...
                    <div
                        className="flex flex-col h-full overflow-auto border-1 sm:w-1/2 sm:self-center rounded-lg gap-4 p-4">
                        {/* <div>
                        ...
                        {cardQuery.data?.data.data.last4 && <Link href={`/settings/update-card`}
                                                                  className="underline flex justify-start text-[#A92223]">Add
                            New Payment Method</Link>}
                        {!cardQuery.data?.data.data.last4 && <div className="flex flex-col items-center gap-4 m-auto ">
                            <div
                                className="px-[2.5rem] py-8 w-max rounded-full text-5xl font-bold bg-red-200 text-[#A92223]">
                                $
                            </div>
                            <p>No Saved Payment Info To Display</p>
                            <Link href={`/settings/update-card`}
                                  className="sm:px-16 px-4   py-2 bg-[#A92223] flex justify-center rounded text-white w-full ">Add
                                New Payment Method</Link>
                        </div>}
                        ...
                        {cardQuery.data?.data.data.last4 && <>
                            ...
                            <button onClick={() => {
                                ...
                                paymentMutation.mutate({
                                    ...
                                    className="m-auto mb-0 px-4 sm:w-max w-full flex justify-center py-2 bg-[#A92223]  rounded text-white">Pay
                                Now
                            </button>
                        </>}
                    ...
                    {(onClose) => (
                        <>
                            ...
                            <ModalBody className="flex flex-col gap-4 pb-8">
                                <p className="text-sm ">You have successfully
                                    booked {packagesQuery.data?.data.data.name}</p>
