...
export default function Checkout(datas: any) {
  ...
  const packagesQuery = useQuery(
    ["packagesIndividual", datas.searchParams.id],
    ({ queryKey }) => axiosInstance.get(`/package?packageId=${queryKey[1]}`),
    { refetchOnWindowFocus: false }
  );
  const cardQuery = useQuery(
    ["cardIndividual"],
    () => axiosInstance.get(`/payment/card`),
    { refetchOnWindowFocus: false }
  );
  const {
    isOpen: isOpen2,
  ...
  const paymentMutation = useMutation(
    (data: PaymentData) => axiosInstance.post("/booking", data),
    {
      onSuccess(data) {
        onOpen2();
      },
  ...
  const couponMutation = useMutation(
    (coupon: string) =>
      axiosInstance.get(
        `/package?packageId=${datas.searchParams.id}&couponCode=${coupon}`
      ),
    {
  ...
  return (
    <>
      {checkoutPage == 0 && (
        <>
          <div
            className={`flex flex-col h-[100vh] sm:h-full gap-4 sm:px-16 px-4 py-8  ${montesserat.className}`}
          >
            ...
            <div className="flex h-full sm:flex-row flex-col sm:w-1/2 sm:self-center border-1 rounded-lg gap-16 p-4">
              <div
                className={`w-full h-full ${
                  !packagesQuery.isFetching ? "overflow-auto" : ""
                }  flex flex-col gap-4`}
              >
                  ...
                  <>
                    <div className="flex flex-col ">
                      ...
                      <p className="font-semibold">
                        {packagesQuery.data?.data.data.package.description}
                      </p>
                    </div>
                    <div className="flex flex-col ">
                      <p className="text-gray-400 text-sm">Package Name</p>
                      <p className="font-semibold">
                        {packagesQuery.data?.data.data.package.name}
                      </p>
                    </div>
                    {/* <div className="flex flex-col ">
                    ...
                    <div className="flex flex-col ">
                      ...
                      <p className="font-semibold">
                        ${packagesQuery.data?.data.data.package.price}
                      </p>
                    </div>
                    <div className="flex justify-between gap-4">
                      <Input
                        ...
                        value={code}
                        isInvalid={couponMutation.isError}
                        errorMessage={"Invalid Coupon"}
                        isDisabled={couponMutation.isSuccess}
                        className="w-full"
                        label="Coupon"
                      ...
                      <Button
                        ...
                        className=" w-full sm:w-max m-auto mb-0 px-4 flex justify-center py-2 bg-[#A92223]  rounded text-white"
                      >
                        Submit Code
                      </Button>
                    </div>
                    <div className="flex flex-col gap-2">
                      ...
                      <div className="flex flex-col gap-2">
                        {couponMutation.data?.data.data && (
                          <div className="flex flex-col gap-2">
                            <div className="flex justify-between border-b-[0.1rem] border-dashed border-[#c9c9c9] pb-4">
                              <p className="text-[#c9c9c9] ">Price</p>
                              <p className="font-semibold">
                                {`$${couponMutation.data?.data.data.discountedPrice}`}
                                <small className="text-red-600">{` ${couponMutation.data?.data.data.coupon.code} applied`}</small>
                              </p>
                            </div>
                          </div>
                        )}
                        <div className="flex flex-col gap-2">
                          <div className="flex justify-between">
                            <p>Tax</p>
                            <p className="font-semibold">
                              ${packagesQuery.data?.data.data.tax}
                            </p>
                          </div>
                        ...
                        <div className="flex flex-col gap-2">
                          ...
                          <div className="flex justify-between">
                            ...
                            <p className="font-semibold">
                              {couponMutation.data?.data.data
                                ? `$${couponMutation.data?.data.data.finalAmount}`
                                : `$${packagesQuery.data?.data.data.finalAmount}`}
                            </p>
                          </div>
                        </div>
                      </div>
          ...
          <div className="flex flex-col gap-4 h-[100vh] sm:h-full sm:px-16 px-4 py-8 ">
            ...
            <div className="flex flex-col h-full overflow-auto border-1 sm:w-1/2 sm:self-center rounded-lg gap-4 p-4">
              ...
              {cardQuery.data?.data.data.last4 && (
                <Link
                  href={`/settings/update-card`}
                  className="underline flex justify-start text-[#A92223]"
                >
              ...
              {!cardQuery.data?.data.data.last4 && (
                <div className="flex flex-col items-center gap-4 m-auto ">
                  ...
                  <Link
                    href={`/settings/update-card`}
                    className="sm:px-16 px-4   py-2 bg-[#A92223] flex justify-center rounded text-white w-full "
                  >
                    Add New Payment Method
                  </Link>
                </div>
                ...
                <>
                  <div className="flex flex-wrap  w-full gap-8">
                    ...
                    <Input
                      ...
                      disabled
                      value={`${
                        cardQuery.data?.data.data.expiryMonth < 10
                          ? `0${cardQuery.data?.data.data.expiryMonth}`
                          : `${cardQuery.data?.data.data.expiryMonth}`
                      }`}
                      placeholder="MM"
                      // defaultValue="12/April/24"
                      labelPlacement="outside"
                  ...
                  <button
                    ...
                    className="m-auto mb-0 px-4 sm:w-max w-full flex justify-center py-2 bg-[#A92223]  rounded text-white"
                  >
                    Pay Now
                  </button>
                </>
            ...
            <>
              ...
              <ModalBody className="flex flex-col gap-4 pb-8">
                <p className="text-sm ">
                  ...
                  {packagesQuery.data?.data.data.name}
                </p>
